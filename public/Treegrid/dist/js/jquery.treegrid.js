...
(function($) {
	...
	function _expand($this) {
		...
		$this.find('>td:first>.treegrid-container>.treegrid-expander')
			.removeClass('treegrid-expander-plus')
	...
	function _initNode($this, depth, forceExpand) {
		...
		$this.each(function() {
			...
			if ($this.data('count')) {
				if ($this.hasClass('expanded')) $expander.addClass('glyphicon-minus');
				else $expander.addClass('glyphicon-plus');
			};
			//indent
	...
	function _expanderClick() {
		var $this = $(this);
		if ($this.hasClass('glyphicon-minus') || $this.hasClass('glyphicon-plus')) {
